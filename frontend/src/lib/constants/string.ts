...
export const BACKEND_URL = isCustomDevelopment
  ? 'http://localhost:3000'
  : process.env.NEXT_PUBLIC_BACKEND_URL || process.env.NEXT_PUBLIC_PROD_BACKEND_URL || 'http://localhost:3000';
...
export const getBackendUrl = (): string => {
  ...
  if (isCustomDevelopment) {
    ...
    return process.env.NEXT_PUBLIC_BACKEND_URL || process.env.NEXT_PUBLIC_PROD_BACKEND_URL || 'http://localhost:3000';
  } else {
